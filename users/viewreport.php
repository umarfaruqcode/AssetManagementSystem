...
include 'config/dbconnection.php';
	if(isset($_GET['id'])){
		$id = $_GET['id'];
		$sql = mysqli_query($mysqli, "SELECT * FROM user_report WHERE id='$id'") or die(mysqli_error($mysqli));
  ...
  <br>
  		<div class="row">
  			<div class="col s8 offset-s2">
  				<div class="card">
            <span class='card-title'>Report <?=$id?></span>
            <div class="card-content">
              <p><b>Asset Name:</b> <?=$row['asset_name'];?><p>
                  <p><b>Subject:</b> <?=$row['subject'];?></p>
                  <br><br>
                  <p><b>Report</b></p>
                  <P><?=$mysqli->escape_string(htmlentities(addslashes(htmlentities(trim($row['report'])))));?></P>
            </div>
